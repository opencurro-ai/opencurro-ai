...
function main(): void {
  ...
  // The multi-agent team runner — routes a team turn (head + members) when the user has enabled
  // multi-agent mode and selected a team. It shares the same providers/tools/stores as the agent.
  ...
  app.use("/api/chat", buildChatRouter(agent, multiAgent, store, config, planApprovals, askQuestions, db));
