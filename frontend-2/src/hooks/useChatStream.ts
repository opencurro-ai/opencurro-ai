...
/** Parse a text/event-stream body and dispatch each event. */
...
export function useChatStream(onFilesChanged?: () => void) {
              ...
              switch (event) {
                ...
                case "tool_call":
                  s.upsertTool(convId, assistantId, {
                    ...
                    status: "running",
                    filePath:
                      (data.args as Record<string, unknown> | undefined)?.file_path as string | undefined,
                  });
                  break;
                case "tool_result": {
  ...
  const stop = useCallback(async () => {
    ...
    if (convId) {
      await fetch(`/api/chat/abort/${encodeURIComponent(convId)}`, { method: "POST" }).catch(() => {});
    }
    useStore.getState().setStreaming(false);
  }, []);
