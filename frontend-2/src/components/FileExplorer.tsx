...
export function FileExplorer() {
  ...
  const openFile = async (path: string) => {
    try {
      ...
      setPreview({ path, content: `Failed to read file: ${e instanceof Error ? e.message : String(e)}` });
    }
  };
    ...
    return (
      <li>
        <button
          ...
          style={pad}
        >
          <ChevronRight className={cn("h-3.5 w-3.5 shrink-0 text-[var(--color-muted)] transition-transform", open && "rotate-90")} />
          {open ? (
